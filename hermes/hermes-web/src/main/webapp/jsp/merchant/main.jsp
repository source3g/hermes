		...
		$(document)
				.ready(
						function() {
							...
							$
									.get(
											"${pageContext.request.contextPath}/merchant/customer/callInStatistics/today/",
											drawPieChart, "json");

							$
									.get(
											"${pageContext.request.contextPath}/merchant/statistics/",
											showStatistics);
							function showStatistics(data) {
								for ( var i = 0; i < data.customerStatistics.length; i++) {
									for ( var j in data.customerStatistics[i]) {
										var str = "<li>"
												+ data.customerStatistics[i][j].displayName
												+ "<span>"
												+ data.customerStatistics[i][j].value
												+ "</span></li>";
								...
								for ( var g = 0; g < data.messageStatistics.length; g++) {
									for ( var k in data.messageStatistics[g]) {
										var str = "<li>"
												+ data.messageStatistics[g][k].displayName
												+ "<span>"
												+ data.messageStatistics[g][k].value
												+ "</span></li>";
