...
public class TcpCommandService implements ApplicationContextAware {
	...
	// LoggerFactory.getLogger(TcpCommandService.class);
	//
	...
	// }
	//
	// public void send(String msgId, String phoneNumber, String content,
	...
	// logger.debug("queueSize:" + tcp.getSndQueueSize() + "unsendSize" +
	// tcp.getUnSend().size() + "command:" + command.getCommand());
	...
	// try {
	// String msgId = result.substring(result.indexOf("msgid=") +
	// "msgid=".length(), result.indexOf("&"));
	// String statemsg = result.substring(result.indexOf("statemsg:=") +
	// "statemsg:=".length(), result.indexOf("&"));
